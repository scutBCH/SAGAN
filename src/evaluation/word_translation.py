...
# DIC_EVAL_PATH = os.path.join(r'D:\dataset\muse', 'dictionaries')
DIC_EVAL_PATH = os.path.join(r'C:\Users\12425\Desktop\SAGAN\data', 'dictionaries')
...
def get_word_pairs(params,lang1, word2id1, emb1, lang2, word2id2, emb2, method, dico_eval):
    """
    Given source and target word embeddings, and a dictionary,
    evaluate the translation accuracy using the precision@k.
    """
    ...
    if method.startswith('csls_knn_'):
        # average distances to k nearest neighbors
        knn = method[len('csls_knn_'):]
        assert knn.isdigit()
        knn = int(knn)
    ...
    results = []
    top_matches = scores.topk(10, 1, True)[1]
    top_matches = top_matches.cpu()
    dico = dico.cpu()
    id2word1 = {v: k for k, v in word2id1.items()}
    id2word2 = {v: k for k, v in word2id2.items()}
    for k in [1]:
        top_k_matches = top_matches[:, :k]
        path = os.path.join(params.exp_path, 'word_pairs.txt')
        with io.open(path, 'w', encoding='utf-8') as f:
            for i in range(dico.shape[0]):
                f.write(id2word1[dico[i][0].item()]+" "+id2word2[top_k_matches[i].item()]+" "+id2word2[dico[i][1].item()]+'\n')
        f.close() 
