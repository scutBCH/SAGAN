...
from src.utils import get_idf, normalize_embeddings
...
class Evaluator(object):
    ...
    def __init__(self, trainer):
        ...
        self.adj_a = trainer.adj_a
        self.adj_ab = trainer.adj_b
        self.gnn_model = trainer.gnn_model
        self.os = trainer.os
        self.ot = trainer.ot
    ...
    def word_translation(self, to_log):
        ...
        # src_emb = self.gnn_model(self.src_emb.weight, self.Adj)

        src_emb = self.mapping(self.os.weight).data
        tgt_emb = self.ot.weight.data

        for method in ['nn','csls_knn_10' ]: # 'csls_knn_10'
    ...
    def word_translation_gcn(self, to_log):
        """
        Evaluation on word translation.
        """
        # mapped word embeddings
    ...
    def word_translation_refine_test1(self, to_log):
        """
        Evaluation on word translation.
        """

        src_emb = self.mapping(self.os.weight[:self.params.refine_max_vocab_eval_acc]).data


        tgt_emb = self.ot.weight.data[:self.params.refine_max_vocab_eval_acc]

        # # try rcr
        # normalize_embeddings(src_emb, "renorm,center,renorm")
        # normalize_embeddings(tgt_emb, "renorm,center,renorm")
    ...
    def dist_mean_cosine(self, to_log):
        ...
        for dico_method in ['nn', 'csls_knn_10']:
            ...
            if dico is None:
                ...
                mean_cosine = (src_emb[dico[:dico_max_size, 0]] * tgt_emb[dico[:dico_max_size, 1]]).clamp(max=1)
                mean_cosine = mean_cosine.sum(1).mean()
    ...
    def dist_mean_cosine_test1(self, to_log):
        """
        Mean-cosine model selection criterion.
        """
        # get normalized embeddings
    ...
    def all_eval(self, to_log):
        ...
        self.word_translation_refine_test1(to_log)
        # self.sent_translation(to_log)
        self.dist_mean_cosine_test1(to_log)
